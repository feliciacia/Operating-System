...
{
    int i;
    unsigned int vpn, offset, j;
    TranslationEntry *entry;
    unsigned int pageFrame;

    // Add variable to store victim page
    int victim;
...
// from the virtual address
	...
	if (vpn >= pageTableSize) {
		...
		// Free page found, store to memory
		if(j < NumPhysPages){
			...
			kernel->virDisk->ReadSector(pageTable[vpn].virtualPage, buffer);
			bcopy(buffer, &mainMemory[j*PageSize], PageSize);
			
		}
		// Not found, store to virtual memory
		else{
			char *buffer1 = new char[PageSize];
			char *buffer2 = new char[PageSize];

			// Run the specified page replacement algorithm
			if(kernel->pageAlgoType == 1)
			{
				victim = fifo % 32;
			}
			else if(kernel->pageAlgoType == 2)
			{
				// Count is incremented each time a page is accessed, the one with least count value will be replaced
				
				int min = pageTable[0].count; // Initialized to the 0-indexed page
				victim = 0;

				// Find the one with least count value
				for(int m =0; m < 32;m++){
					if(min > pageTable[m].count){
						min = pageTable[m].count;
						victim = m;
					}
				}
				// Page is accessed, count incremented
				pageTable[victim].count++;
			}		
			else if(kernel->pageAlgoType == 3)
			{
				victim = rand() % 32;
			}

			printf("Number of %d page swap out\n", victim);

			bcopy(&mainMemory[victim*PageSize], buffer1, PageSize);
			kernel->virDisk->ReadSector(pageTable[vpn].virtualPage, buffer2);
			bcopy(buffer2, &mainMemory[victim * PageSize], PageSize);
			kernel->virDisk->WriteSector(pageTable[vpn].virtualPage, buffer1);
