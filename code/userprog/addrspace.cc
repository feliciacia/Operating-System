...
*/
    // Page table is defined here rather in constructor
		...
		for(unsigned int i=0, j=0; i<numPages; i++){
			...
			if(j < NumPhysPages){
				...
				kernel->machine->main_tab[j] = &pageTable[i];
        			...
        			pageTable[i].count++;
			executable->ReadAt(
			&(kernel->machine->mainMemory[j * PageSize]), PageSize, noffH.code.inFileAddr + (i* PageSize));
    			}
			else{
				char* buf;
				...
				kernel->virDisk->WriteSector(k, buf);
